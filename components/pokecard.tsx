...
function renderTypes(pokemon: pokecardProps | undefined) {
  ...
  return pokemon.type.map((element) => {
    ...
    return (
      <li
        className={`${typeColor} font-bold text-white text-center p-1 rounded-lg font-arcade`}
        key={pokemon.id}
      >
        {element.type.name}
      </li>
...
export default function Pokecard(id: { id: number }) {
  ...
  return (
    <>
      {!isLoading ? (
        <div
          className="bg-red-500 p-2 rounded-lg justify-between flex flex-col shadow-lg cursor-pointer"
          key={pokemon?.name}
          onClick={() => router.push("/" + (pokemonId - 1))}
        >
          <div className={`flex  justify-between `}>
            <h1 className="text-2xl font-arcade text-white">{pokemon?.name}</h1>
            <h2 className="text-2xl font-arcade text-white">
          ...
          <div className="flex justify-between items-center ">
            <img
              ...
              height={60}
            ></img>
            <ul className=" flex flex-col gap-1 ">{renderTypes(pokemon)}</ul>
          </div>
          ...
          <div className="mt-6">
            ...
            <ul className="flex gap-5">
              {pokemon?.abilities.map((abil, index) => {
                return (
                  <li className="text-white font-arcade" key={pokemon.id}>
                    {abil.ability.name}
                  </li>
                );
              })}
